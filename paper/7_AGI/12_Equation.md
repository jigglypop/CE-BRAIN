...
이 문서는 런타임 기호를 모으는 문서지만, `paper/README.md`와 `paper/6_뇌/05_실험근거.md`를 기준으로 읽어야 한다. 아래 5계층 스택만 현재 canonical runtime spec 이고, 그 아래의 나머지 방정식은 보조 유도나 설계 탐색으로 읽는다.
...
- `paper/6_뇌/05_실험근거.md`에서 `supported`인 현상만 위 stack의 대응 근거로 사용한다.
...
### 7.7 부트스트랩 수렴 — $p^*$ 명시 삽입 선형화의 수렴률 예시

아래 표는 A.2의 사상으로부터의 유도가 아니라, 고정점 $p^*$를 명시적으로 삽입한 선형화 반복 $p^+ = p^* + 0.155\,(p - p^*)$의 수렴률 예시다(20-audit §2, `route_b_corrections.log` C2 — 표 수치와 소수점 일치).
...
이 선형화는 $p^*$를 갱신식에 직접 넣었으므로 3회 반복에서 $p^*=(4.87\%,\;26.2\%,\;68.9\%)$에 $0.4\%$ 이내로 접근한다. 이는 수렴률 예시이지 신경 동역학이 $p^*$를 생성한다는 예측이 아니다.
...
[정의] 부트스트랩 사상은 스칼라 $a$-성분에 한정한다.

$$B_a(a) = e^{-(1-a)D_{\text{eff}}}, \qquad a \in (0,1)$$

#### A.2.2 스칼라 수축 정리

[정리] $a^* = 0.0487077$은 $B_a(a) = e^{-(1-a)D_{\text{eff}}}$의 $(0,1)$ 내 유일 내부 고정점이며, 수축률은

$$|B_a'(a^*)| = D_{\text{eff}}\,a^* = 0.1547 < 1$$

이므로 $a^*$의 열린 근방에서 Banach 의미의 수축 수렴이 성립한다.

증명한다. $F(a)=e^{-D_{\text{eff}}(1-a)}-a$로 두면 $F(0)>0$, $F(1)=0$, $F''(a)=D_{\text{eff}}^2e^{-D_{\text{eff}}(1-a)}>0$이다. 유일 임계점 $a_c=1-\log D_{\text{eff}}/D_{\text{eff}}$에서 $F(a_c)=(1+\log D_{\text{eff}})/D_{\text{eff}}-1<0$이므로, 엄격 볼록성에 의해 영점은 $(0,a_c)$의 하나와 경계점 $1$뿐이다. 내부 영점을 $a^*$라 하면 $B_a'(a^*)=D_{\text{eff}}a^*=0.1547<1$이므로 연속성에 의해 $a^*$의 어떤 근방에서 $B_a$는 수축이다. $\square$

**[공리: 경험식]** 나머지 성분 $(s,\;b) = (0.2623,\;0.6891)$은 $B_a$의 산출이 아니라 경험식 공리로 도입한다. 물리 정본 `paper/3_상수/9_우주론_수식_의미와_후보.md` §2는 암흑 성분 분할이 별도 stress tensor를 요구한다고 명시하며, `paper/9_등호이전/05o_CE_residual_cosmology_forward_model.md` §1.1은 $\Omega_{\rm DM}/\Omega_\Lambda$ 분할을 경험식으로 등재한다. 사망 조건: $(0.2623,\;0.6891)$을 무입력 전개로 유도하는 정본 문서가 발견되면 이 라벨은 해당 유도로 교체된다(현재 근거: 12-routes B.4의 전 docs grep 음성 — 15.1 표의 "출처 상수 $e,\pi,0$" 표기는 유도 없이 붙은 라벨).

#### A.2.3 자기조직화 브리지의 검증 조건

[미완성] 다음 목록은 스칼라 사상 $B_a$를 신경 모듈의 점유율 동역학과 연결하기 위한 검증 체크리스트다. 조건을 관측하는 것만으로 실제 점유율 갱신이 $B_a$와 같다는 결론은 나오지 않는다. 격상에는 점유율 전이 연산자 $T_a$를 먼저 정의하고, 사전 고정한 정의역에서 $T_a=B_a$ 또는 오차가 제어된 근사임을 별도로 증명해야 한다.
...
3. **국소 안정성**: $|B_a'(a^*)|=D_{\text{eff}}a^*<1$에 대응하는 섭동 후 수렴률을 측정한다.
...
이 표는 게이트 `F1`을 시험하는 경로다. 5조건 중 하나라도 결손이거나 $T_a$와 $B_a$의 사상이 증명되지 않으면 자기수렴 주장은 [미완성] 또는 사전등록된 [예측]으로만 남는다.
