...
이 총론은 CE 뇌 연구에서 빌려온 구조를 AGI 구현 후보로 번역할 때, 무엇이 구현 명세이고 무엇이 경험적 효능 가설인지를 분리한다. 독자는 이 주제를 1년 전에 처음 배우던 저자 자신을 전제로 하며, 신경망 학습의 기본 용어와 분리된 수학 정리·실험 결과·설계 선택의 차이를 알고 있으면 된다.

먼저 현재 runtime과 합성 과제의 판정을 입력으로 받아 범위를 고정하고, 다음으로 다섯 설계 원리와 각 장의 입출력을 연결한다. 이후 비교표와 전제 절에서 성공은 사전 등록한 대조군·OOD·ablation gate를 통과하는 것으로, 실패는 그 gate의 재현 가능한 미통과로 판정한다는 읽기 순서를 제시한다.
...
핵심 논제가 구현 후보로 의미를 가지려면, 비교 대상인 현재 AI의 입력·출력과 결함을 먼저 같은 수준에서 놓아야 한다. 아래 표는 생물학적 비유를 기능 요구로 번역한 비교일 뿐 뇌와 모델이 동일하다는 경험적 결과가 아니며, 각 후보의 효능은 뒤 장의 OOD 및 ablation 대조로 반증 가능해야 한다.
...
앞의 결함은 하나의 성능 주장으로 해결되지 않으므로, 이 절은 각 원리가 받는 상태·학습 신호와 내놓는 제약 또는 업데이트를 장별로 분리한다. 시간 단위는 3장의 training step 또는 수면 주기이며, 생물학적 명칭은 구현의 timebase를 대체하지 않는다.
...
각 원리는 독립적인 증거가 아니라 다음 장의 입력을 만드는 설계 계층이므로, 독자는 표의 장 번호 순서로 의존성을 확인해야 한다. 코드가 있는 행은 재현 가능한 구현 위치를 뜻할 뿐 해당 코드가 AGI 성능이나 생물학적 대응을 입증했다는 표시는 아니다.
...
비교는 채널·활성 비율처럼 설계에 들어가는 제약과 실제 벤치마크 우위를 구별하는 데 필요하다. 표의 장점은 구현 가설 또는 조건부 비용 추정이며, 기준선보다 나은지 여부는 동일 예산의 holdout·OOD·ablation 결과가 없으면 미완성으로 남는다.
...
이제 설계 입력과 출력 사이에 남은 외부 가정을 명시하면, 실패한 gate가 어느 비유·연산자·데이터 경로를 기각하는지도 추적할 수 있다. 특히 수면, STDP, 희소성의 명칭은 생물학적 기제의 주장으로 승격되지 않으며, 시간척도와 단위는 각 구현의 step·token·wall-clock 측정으로 별도 보고해야 한다.
