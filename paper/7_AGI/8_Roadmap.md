...
이 문서는 2--7장의 설계 후보를 구현 단계와 벤치마크 gate로 배열한 계획 문서다. 독자는 앞 장의 아키텍처·수면·희소성·국소학습·환각 억제·메타인지 명세를 입력으로 읽으며, 각 단계의 출력은 deliverable과 측정 기록이지 미래 완료를 미리 주장하는 결과가 아니다.

현재 상태와 substrate 한계를 먼저 확인한 뒤 단계별 prerequisite·gate·rollback, benchmark, risk, 성공·중단 규칙 순으로 읽는다. 일정은 계획 timebase이고 성공은 사전 고정 baseline·holdout·OOD·ablation 통과이며, 실패 또는 STOP은 다음 단계로의 승격을 막는 명시적 산출이다.
...
로드맵의 모든 단계는 적용 substrate와 현재 측정의 범위를 먼저 고정할 때만 해석할 수 있다. 아래 기록은 구현·관찰·계획을 분리하며, 수치가 있어도 다른 모델 규모나 실제 AGI로 외삽하지 않는다.
...
코드베이스의 상태는 파일 존재 여부와 벤치마크 효능을 구분해 기록한다. 표의 구현 완료는 deliverable의 존재를 뜻하며 성능·생물학·AGI 완료 주장이 아니다.
...
폐쇄 판정은 해당 route의 사전 조건 또는 gate가 충족되지 않았음을 뜻한다. 이는 대체 단계의 성공이나 전체 연구 방향의 반증이 아니라, 기록한 prerequisite와 rollback을 보존하는 STOP 산출이다.
...
다음 선택지는 완료된 결과가 아니라 자원·substrate·검증 계약에 조건부인 계획이다. 어느 경로도 holdout·OOD·ablation gate 전에는 성공으로 승격하지 않는다.
...
현재 평가는 코드 deliverable, 미구현 의존성, 이미 실행한 gate를 분리한다. 표의 상태는 다음 phase의 prerequisite를 정하며 일정 예측이나 일반 성능 결론을 제공하지 않는다.

### 1.1 구현 완료

완료 표는 코드·artifact가 존재하고 지정된 구현 범위를 충족한다는 deliverable 기록이다. 효능과 일반화는 별도 benchmark gate 전에는 계획 또는 미완성으로 유지한다.
...
미구현 표는 다음 phase가 받는 결손 입력과 prerequisite를 명시한다. 우선순위는 일정 제안이며 gate 통과나 자원 확보를 뜻하지 않는다.
...
각 phase는 이전 deliverable을 입력으로 받고 구현물·측정 artifact·gate 판정을 출력으로 남긴다. prerequisite가 깨지거나 rollback 조건이 발생하면 일정과 상관없이 다음 단계로 넘어가지 않는다.
...
현재 구현/효능/진단의 정본은 `검증_원장/AGI_STDP_Efficacy_Audit.md`다. 구현 완료를
...
벤치마크는 계획의 성패를 정하는 외부 판정 계약이다. metric의 ground-truth provenance, 동일 baseline, OOD split, component ablation을 고정하지 않은 측정은 진행 보고일 뿐 성공 증거가 아니다.

### 3.1 핵심 지표

핵심 지표는 각 phase의 output을 baseline과 비교하는 측정 계약이다. provenance가 있는 holdout·OOD·ablation 없이 표의 예측은 완료 주장이 아니다.
...
비교 대상은 같은 예산과 데이터 조건의 외부 baseline을 고정하기 위해 둔다. 단일 내부 모델과의 비교는 성공 gate를 대체하지 않는다.
...
모델 규모는 compute·memory·wall-clock timebase를 맞추는 통제 변수다. 규모 차이로 생긴 결과를 구조 효과로 해석하지 않는다.
...
리스크 표는 예상 문제를 완료 주장으로 덮지 않고, 감지 metric과 rollback 경로를 사전에 남긴다. 완화가 실패하면 해당 phase는 중단 또는 재설계 대상으로 판정한다.
...
의존성은 각 deliverable이 어떤 선행 구현·측정 gate를 요구하는지 나타낸다. 그래프의 화살표는 논리적 prerequisite이며 일정상 병렬 가능성과 성공 보장을 뜻하지 않는다.
...
성공 기준은 계획·구현·실험 결과를 가르는 gate다. 각 기간의 수치는 목표 일정일 뿐 증거가 아니며, baseline·OOD·ablation의 어느 하나가 미통과하면 rollback 또는 STOP으로 기록한다.

### 6.1 단기 (Phase 1-3, 약 9주)

단기 기준은 계획된 deliverable의 최소 통과 gate다. 아래 항목은 baseline·holdout·ablation에서 모두 확인되지 않으면 완료가 아니라 rollback 사유가 된다.
...
중기 기준은 위험 metric과 학습 규칙의 효능을 분리해 판정한다. OOD 악화나 false-positive 비용 증가는 개선 수치가 있어도 실패 조건이다.
...
장기 기준은 self-monitoring 모듈의 관측 가능한 안정성 효과에 한정한다. 철학적 의식이나 AGI 완료는 이 gate가 판정할 수 없는 범위다.
...
실패 규칙은 어느 prerequisite·metric·ablation이 깨졌는지를 기록하고 다음 단계 승격을 막기 위한 rollback 계약이다. 사후 metric 교체로 등록된 중단 조건을 제거하지 않는다.
...
마지막 요약은 문서 간 기능 대응을 압축한 설계 지도다. 아래 표현은 수학적 동일성·생물학적 증거·AGI 완료 선언이 아니라, 앞 절의 조건부 계획과 미완성 경계를 함께 읽어야 하는 비유다.
