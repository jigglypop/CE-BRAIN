...
| 강제 | 사람이 지키라고 쓰지 말고 훅(결정적 코드)이 막는다 | 훅 7종, `ledger.py`·`verify_derivation.py`·`safety_screen.py`(생물학 opus 라우팅 포함) |
...
```
CLAUDE.md            프로젝트 지침
settings.json        permissions + 훅 7종
agents/              domain-classifier | prover adversary sourcer judge paper-writer | prover-bio adversary-bio sourcer-bio judge-bio paper-writer-bio bio-reader
...
hooks/
  ...
  safety-screen.cmd  PreToolUse Agent|Write|Edit|Bash → lib/safety_screen.py (범위 밖 위험 범주·모델 내부 추출 차단, 도메인 판별 영수증 없는 연구 역할 호출 차단, 생물 내용은 생물 전담(opus)으로만 라우팅, 과대 프롬프트 경고. 자기 정의 파일만 예외)
