...
자료 감사를 연구 자체로 팽창시키지 않는다.

1. 논문 Methods, README, metadata, schema와 변수표만으로 먼저 판정한다.
2. 최종 사슬에 필수인 직접 미시변수, 독립 output-likelihood 계량, 별도 행동,
   동일 생물단위 identity와 필요한 개입 중 하나라도 없으면 다운로드 전에
   `NOT_ELIGIBLE_FOR_CHAIN`으로 정지한다.
3. eligibility 확인용 sample은 원칙적으로 100MB 이하로 제한한다.
4. 500MB 초과 다운로드, 전체 cohort, 전수 checksum은 사용자 별도 승인과
   그 용량이 여는 정확한 endpoint가 있을 때만 허용한다.
5. 차단된 자료의 문턱·창·제외·split을 완화해 원 confirmatory 지위를 구제하지
   않는다.
6. 이미 확보한 원자료는 삭제하지 않는다. 삭제는 범위와 복구 가능성을 별도로
   확인한다.

자료가 위 gate를 통과하지 못하면 다른 대용량 자료를 연쇄 탐색하지 않고,
필요한 신규 실험의 최소 측정·개입 사양을 산출한다.
...
endpoint 편향을 구체적으로 보이지 못하면 병렬 부채로 남긴다. 같은 층의 감사나
재실행은 새 실패 증인 없이 반복하지 않는다.
