"""NaN audit copy of shift_operators.py (only change: zero-length vectors excluded from R, counted).
...
def main():
    ...
    result = {"schema": "ce-a1-shift-operators-v1-nan-audit", "code_sha256": code_hash, "eyes": {}}
    for side in ("R", "L"):
        eye = {}
        for family, (plus, minus) in PAIRS.items():
            for sub in "abcd":
                ...
                unit = v[norms > 0] / norms[norms > 0, None]
                ...
                eye[f"{family}{sub}"] = {"neurons": len(v), "median_vector": med.tolist(),
                                         ...
                                         "median_magnitude": float(np.median(np.linalg.norm(v, axis=1))), "zero_length_vectors": zero}
        tests = {}
        for family in PAIRS:
    ...
    with (HERE / "results_shift_operators_nan_audit.json").open("x", encoding="utf-8") as stream:
