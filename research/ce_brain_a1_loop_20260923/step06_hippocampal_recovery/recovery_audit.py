"""AUDIT copy (only change: session eligibility counts valid, choice-resolved trials in all three cue windows).
...
def main():
    ...
    for region in ("CA1", "PFC"):
        ...
        for s in win["sessions"]:
            ...
            trials = s["trials"]
            usable = np.array([t["choice_reason"] == "resolved" and all(t["windows"][w]["reasons"] == [] for w in ("precue", "earlycue", "latecue")) for t in trials])
            correct = np.array([bool(t["correct"]) for t in trials]) & usable
            odor = np.array([t["odor_side"] == "left" for t in trials])
    ...
    result = {"schema": "ce-a1-step06-hippocampal-recovery-audit", "code_sha256": code_hash, "npz_sha256": NPZ_SHA,
              "verdict": verdict, "regions": per_region}
    with (HERE / "results_audit_eligibility.json").open("x", encoding="utf-8") as stream:
