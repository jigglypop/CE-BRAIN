...
def harmonics(v, epg, angles):
    ...
    return [float(abs(np.sum(ve * np.exp(-1j * m * angles))) / norm) if norm > 0 else 0.0 for m in range(3)]
...
def main():
    ...
    result = {"schema": "ce-a1-step02-ring-spectrum", "code_sha256": code_hash, "verdict": verdict,
