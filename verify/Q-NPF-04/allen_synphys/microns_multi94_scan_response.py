...
def main():
    ...
    targets=sorted([t for t in all_targets if (t['session'],t['scan_idx'])==(9,4)],key=lambda t:(t['field'],t['mask_id']))
    assert len(targets)==45 and len({t['nucleus_id'] for t in targets})==36
    save('microns_multi94_scan_contract.json',dict(question='Open the next lexicographically ordered scan with registered structural targets and verify the same mask-domain/time/first1250-frame input checks.',
        selection='session9 scan4; all45 ROIs for36 nuclei targets, no response selection; zero nuclei overlap with first scan, same mouse17797.',
    ...
    mapping=decode();meta=json.loads((HERE/'microns_scan_9_4_asset.json').read_text())
    raw.URL=next(u for u in meta['contentUrl'] if u.startswith('https://dandiarchive.s3.amazonaws.com/'))
    raw.CACHE=ROOT/'data/external/microns_functional_nwb/scan_9_4_ranges';raw.LIMIT=64*1024*1024
    ...
    fields=[];coordinates=[];spans=set();values=np.empty((1250,45));times=None;trial_counts={}
    with raw.CachedRanges() as reader:
        with h5py.File(reader,'r') as f:
            for field in sorted({t['field'] for t in targets}):
                ...
                expected=sorted(r['mask_id'] for r in mapping.values() if (r['session'],r['scan_idx'],r['field'])==(9,4,field))
    ...
    output=ROOT/'data/external/microns_functional_nwb/scan_9_4_first1250.npz'
