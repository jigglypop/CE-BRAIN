"""WT session 6: 과거 상태 모형과 표적별 잔차 보정의 시간 분할 비교."""
import json
...
def main():
    ...
    sid = support['summary']['exported_data']['policies']['event_local']['selected_first_numeric_session']
    assert sid == '6'
    ...
    alignment_path = HERE / 'randi_raw_alignment_result.json'
    alignment = json.loads(alignment_path.read_text(encoding='utf-8'))
    assert all(r['alignment_pass'] for r in alignment['sessions'])
    ...
    contract = {'question': '비표적 세포의 미래 반응 예측에 자극 대상 정보가 이득을 주는가',
                'selection': '메타데이터 지원 게이트 통과 중 숫자 순 첫 WT 세션; unc31은 지원 부족으로 미평가',
                'support_sha256': sha(support_path), 'split': split,
                'raw_alignment_sha256': sha(alignment_path),
                'additional_preprocessing': 'none: no interpolation, smoothing, global photobleaching or spike removal; original raw extraction history not independently verified',
    ...
    save('randi_raw_prediction_session6_contract.json', contract)
    ...
    result = {'contract_sha256': sha(HERE / 'randi_raw_prediction_session6_contract.json'),
    ...
    save('randi_raw_prediction_session6_result.json', result)
