...
def main():
    ...
    save('icms93_posttrain_contract.json',dict(question='In this exploratory session, how does post-train stored firing differ from prestimulus firing and the same nominal catch windows?',
    ...
    with h5py.File(path,'r') as f:
        ...
        for quality in ('good_only','all_quality'):
            ...
            for current in sorted(set(currents)):
                mask=eligible&(currents==current);assert mask.any();pre=counts[mask,:,0].mean(axis=0)/.5;post=counts[mask,:,1].mean(axis=0)/.5;delta=post-pre;contrast=delta-cdelta
    ...
    dest=HERE/'icms93_posttrain_counts.npz'
    ...
    save('icms93_posttrain_result.json',dict(trials=rows,per_unit=perunit,summaries=summaries,session_proxy=[lower,upper],geometry_failures=sum(not r['geometry_pass'] for r in rows),counts_sha256=sha(dest),code_sha256=sha(Path(__file__))))
