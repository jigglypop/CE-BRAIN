...
def main():
    save('microns_fourth_scan_contrast_contract.json',dict(question='Apply the existing first-window structural contrast to the next scan5/7 with35 nonoverlapping nuclei in the same mouse.',
    ...
    meta=json.loads((HERE/'microns_fourth_scan_result.json').read_text());assert meta['all_finite'] and meta['nonconstant']==35
    path=ROOT/'data/external/microns_functional_nwb/scan_5_7_first1250.npz';assert sha(path)==meta['artifact_sha256']
    ...
    for mode in ('common_frame','positive_delay_sensitivity'):
        ...
        assert len(pairs)==595
        results.append(dict(timing=mode,cells=35,dyads=595,**contrasts(pairs)))
        outputs.append(dict(timing=mode,pairs=pairs))
    save('microns_fourth_scan_contrast_pairs.json',outputs)
    save('microns_fourth_scan_contrast_result.json',dict(results=results,fixed_distance_boundaries_um=cuts.tolist(),limits='Conditional registered-cell descriptive observation in same mouse; not replicated causal mechanism.'))
