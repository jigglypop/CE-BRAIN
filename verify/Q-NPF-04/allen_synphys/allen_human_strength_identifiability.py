...
save('allen_human_strength_identifiability_contract.json',{
    'question':'보유 Allen E-E 기록은 상호연결 강도와 송신·수신 세포 효과를 분리할 관측 구조를 제공하는가?',
    'primary':'human TCx ex/ex target_layer 양쪽2,2/3,3; 양방향has_synapse비결측 및 n_ex_test_spikes>10',
    'secondary':'별도 가용성 조사로 human TCx ex/ex 모든층; 상층 재현으로 합치지 않음; 효과값 선택 없음',
...
with sqlite3.connect(DB.as_uri()+'?mode=ro',uri=True) as db:
    ...
    rows=[dict(r) for r in db.execute('''select p.id,p.experiment_id,p.pre_cell_id,p.post_cell_id,p.has_synapse,p.n_ex_test_spikes,
      ...
      where s.species='human' and e.target_region='TCx' and a.cell_class_nonsynaptic='ex' and b.cell_class_nonsynaptic='ex' ''')]
...
for scope in ('upper_layers','all_layers_separate_inventory'):
    ...
    assert counts['qualified_directions'] % 2 == 0
...
save('allen_human_strength_identifiability_result.json',{'contract_sha256':sha(HERE/'allen_human_strength_identifiability_contract.json'),
    'scopes':results,'verification':'PASS: bidirectional denominator and independent projections'})
