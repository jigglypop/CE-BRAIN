...
def main():
    specs = [('9_6','microns_last_trial_level_contract.json','microns_last_trial_level_result.json','microns_last_scan_contrast_pairs.json')]
    inputs = []
    ...
    save('microns_last_within_shift_contract.json',dict(
        ...
        method='Same LOO condition-time residual, subtract eachwithin-trial trial mean. Normalize by original total residual SD products, fixed per-scan field-distance weights. 999 independent per-cell circular shifts; same shift for all conditions and within-trial samples.',
        selection='Last listed scan9/6; both timings; all10/first5/last5; all20 registered targets. All6 scenarios; no favorable selection.',
    ...
    save('microns_last_within_shift_result.json',dict(results=out,status='DESCRIPTIVE_WITHIN_TRIAL_ALIGNMENT_CONTROL',limits='Not calibrated p-values or independent animal replication.'))
    for r in out:
