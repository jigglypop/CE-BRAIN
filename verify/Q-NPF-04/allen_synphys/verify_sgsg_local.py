...
run.STORE = run.source.base.ROOT / 'data/external/sgsg_local_calibration'
run.CONTRACT = run.HERE / 'sgsg_local_calibration_contract.json'


def main():
    selection = json.loads((run.HERE / 'sgsg_local_selection.json').read_text())
    result = json.loads((run.HERE / 'sgsg_local_result.json').read_text())
    contract = json.loads(run.CONTRACT.read_text())
    assert selection['contract_sha256'] == run.source.base.sha(run.CONTRACT)
    assert result['selection_sha256'] == run.source.base.sha(run.HERE / 'sgsg_local_selection.json')
    ...
    output = dict(result_sha256=run.source.base.sha(run.HERE / 'sgsg_local_result.json'),
    ...
    run.source.write_once(run.HERE / 'sgsg_local_verification.json', output)
