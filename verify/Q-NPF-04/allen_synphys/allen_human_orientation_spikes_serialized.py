...
save('allen_human_orientation_spikes_serialized_contract.json',{
    'repair':'Original computation passed checks but JSON serialization failed on numpy int64; cast count only; preserve original code/contract/partial result',
...
out={'contract_sha256':sha(HERE/'allen_human_orientation_spikes_serialized_contract.json'),'dyads':len(edges),'slices_with_oneway_edges':len(folds),
     ...
     'slices_improved':int(sum(f['logloss']<np.log(2)-1e-12 for f in folds)),
...
save('allen_human_orientation_spikes_serialized_result.json',out)
