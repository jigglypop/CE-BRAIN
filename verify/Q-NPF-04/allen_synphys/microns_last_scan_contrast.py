...
def main():
    save('microns_last_scan_contrast_contract.json',dict(question='Apply the existing first-window structural contrast to the next scan9/6 with20 nuclei including one overlapping first-scan nuclei in the same mouse.',
    ...
    meta=json.loads((HERE/'microns_last_scan_result.json').read_text());assert meta['all_finite'] and meta['nonconstant']==20
    path=ROOT/'data/external/microns_functional_nwb/scan_9_6_first1250.npz';assert sha(path)==meta['artifact_sha256']
    ...
    for mode in ('common_frame','positive_delay_sensitivity'):
        ...
        results.append(dict(timing=mode,cells=20,dyads=190,**contrasts(pairs)))
        outputs.append(dict(timing=mode,pairs=pairs))
    save('microns_last_scan_contrast_pairs.json',outputs)
    save('microns_last_scan_contrast_result.json',dict(results=results,fixed_distance_boundaries_um=cuts.tolist(),limits='Conditional registered-cell descriptive observation in same mouse; not replicated causal mechanism.'))
