...
def main():
    specs = [('4_7','microns_first_trial_level_contract.json','microns_first_trial_level_result.json','microns_structure_response_pairs.json'),
             ('5_3','microns_trial_level_contract.json','microns_trial_level_result.json','microns_next_scan_contrast_pairs.json')]
    inputs = []
    ...
    save('microns_within_shift_contract.json',dict(
        ...
        selection='Both scans, both timings, all10/first5/last5; scan4/7 include/exclude3151; scan5/3 additionally last_without10. All20 scenarios, no favorable selection.',
    ...
    save('microns_within_shift_result.json',dict(results=out,status='DESCRIPTIVE_WITHIN_TRIAL_ALIGNMENT_CONTROL',limits='Not calibrated p-values or independent animal replication.'))
    for r in out:
