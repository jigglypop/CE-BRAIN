...
def main():
    save('microns_tenth_scan_contrast_contract.json',dict(question='Apply the existing first-window structural contrast to the next scan7/4 with52 nuclei including three overlapping first-scan nuclei in the same mouse.',
    ...
    meta=json.loads((HERE/'microns_tenth_scan_result.json').read_text());assert meta['all_finite'] and meta['nonconstant']==52
    path=ROOT/'data/external/microns_functional_nwb/scan_7_4_first1250.npz';assert sha(path)==meta['artifact_sha256']
    ...
    for mode in ('common_frame','positive_delay_sensitivity'):
        ...
        results.append(dict(timing=mode,cells=52,dyads=1326,**contrasts(pairs)))
        outputs.append(dict(timing=mode,pairs=pairs))
    save('microns_tenth_scan_contrast_pairs.json',outputs)
    save('microns_tenth_scan_contrast_result.json',dict(results=results,fixed_distance_boundaries_um=cuts.tolist(),limits='Conditional registered-cell descriptive observation in same mouse; not replicated causal mechanism.'))
