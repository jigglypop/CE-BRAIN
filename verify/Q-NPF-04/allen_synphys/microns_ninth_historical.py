...
def main():
    ...
    old=[r for r in old if (int(r['session']),int(r['scan_idx']))==(7,3)]
    ...
    save('microns_ninth_historical_result.json',dict(rows=rows,code_sha256=sha(Path(__file__)),old_sha256=sha(ROOT/'data/external/microns_coregistration_v343/functional_coreg_merged.csv.gz'),limits='Historical provenance check only; does not adjudicate current biological identity or replace v1412 assignment.'))
