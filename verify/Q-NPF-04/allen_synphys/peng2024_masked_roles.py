...
save('peng2024_masked_roles_contract.json',{
...
for width,name in ((1,'pia'),(2,'pia_role')):
    ...
    for patient in sorted(set(pids)):
        ...
        for _ in range(100):
            ...
            while objective(beta-scale*step)>objective(beta)-1e-4*scale*float(g@step):
...
out={'contract_sha256':sha(HERE/'peng2024_masked_roles_contract.json'),'directions':len(edges),'patients':len(set(pids)),
...
save('peng2024_masked_roles_result.json',out)
