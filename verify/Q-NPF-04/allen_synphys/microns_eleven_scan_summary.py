...
def main():
    specs=[('4_7','microns_first_trial_level_result.json'),
           ...
           ('6_6','microns_seventh_trial_level_result.json'),
           ('6_7','microns_eighth_trial_level_result.json'),
           ('7_3','microns_ninth_trial_level_result.json'),
           ('7_4','microns_tenth_trial_level_result.json'),
           ('8_5','microns_eleventh_trial_level_result.json')]
    rows=[];sources=[]
    ...
    save('microns_eleven_scan_summary_result.json',dict(results=rows,sources=sources,
        cells_by_scan={s:len(v) for s,v in ids.items()},unique_nuclei=len(set.union(*ids.values())),overlaps=overlaps,
        code_sha256=sha(Path(__file__)),limits='Same animal, scan-specific populations and denominators. No pooled significance test or independent animal replication. Includes all eleven completed ordered scans; incomplete16-scan coverage.'))
