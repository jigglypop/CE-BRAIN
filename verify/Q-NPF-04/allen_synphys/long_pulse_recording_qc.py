...
save('long_pulse_recording_qc_contract.json',{
...
with raw.CachedRanges() as reader:
    with h5py.File(reader,'r') as f:
        ...
        values=nb['numericalValues'][:,[keys.index(k) for k in columns],:]
        merged={}
        for row in values:
...
out={'contract_sha256':sha(HERE/'long_pulse_recording_qc_contract.json'),'new_bytes':new_bytes,'records':rows,
...
save('long_pulse_recording_qc_result.json',out)
