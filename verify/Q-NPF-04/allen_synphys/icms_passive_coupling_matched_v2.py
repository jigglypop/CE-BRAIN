...
def main():
    ...
    save('icms_passive_coupling_matched_v2_contract.json',dict(code_sha256=sha(Path(__file__)),input_sha256=sha(p),
    ...
    for animal in sorted({r['animal'] for r in results}):
        ...
        summaries.append(dict(animal=animal,total_conditions=len(rr),defined_conditions=len(defined),defined_sessions=len(sessions),
            sign_disagreement=sum(bool(np.sign(r['pc']['npl_minus_pl'])!=np.sign(r['pc_norm']['npl_minus_pl'])) for r in defined),
    ...
    save('icms_passive_coupling_matched_v2_result.json',dict(conditions=results,sessions=session_results,animals=summaries))
