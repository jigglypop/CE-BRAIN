...
def main():
    ...
    pairs=json.loads((HERE/'microns_last_scan_contrast_pairs.json').read_text())[0]['pairs'];a=np.array([lookup[p['unit_a']] for p in pairs]);b=np.array([lookup[p['unit_b']] for p in pairs]);simple,matched=weights(pairs)
