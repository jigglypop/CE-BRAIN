...
HERE=source.HERE
BASE=source.BASE
STORE=BASE.ROOT/'data/external/masked_neighbor_prediction_polished'
...
def initial_fit(train_a,train,c,f,columns):
...
"""동일 ridge 목적함수의 희소 설계행렬 Newton 보정 함수."""


def fit(train_a,train,c,f,columns):
    from scipy import sparse
    theta,mean,scale,initial=initial_fit(train_a,train,c,f,columns)
    n=len(train_a);u,v=np.nonzero(train);y=train_a[u,v].astype(float)
    x=(f[u,v][:,columns]-mean)/scale;m=len(u);r=np.arange(m)
    rr=[r,r,r,r];cc=[np.zeros(m,dtype=int),1+u,1+n+v,1+2*n+c[u,v]];values=[np.ones(m)]*4
    for k in range(len(columns)):
        rr.append(r);cc.append(np.full(m,1+2*n+6+k));values.append(x[:,k])
    design=sparse.coo_matrix((np.concatenate(values),(np.concatenate(rr),np.concatenate(cc))),shape=(m,len(theta))).tocsr()
    penalty=np.ones(len(theta));penalty[0]=0
    history=[]
    for iteration in range(4):
        z=design@theta;p=expit(z)
        g=np.asarray(design.T@(p-y)).ravel()+penalty*theta
        error=float(np.max(abs(g)));history.append(error)
        if error<=1e-7 or iteration==3:break
        h=(design.T@design.multiply((p*(1-p))[:,None])).toarray()
        h.flat[::len(theta)+1]+=penalty
        theta-=np.linalg.solve(h,g)
    loss=float(np.sum(np.logaddexp(0,z)-y*z)+.5*np.dot(theta[1:],theta[1:]))
    assert loss<=initial['penalized_training_loss']+1e-7
    return theta,mean,scale,dict(gate_pass=bool(error<=1e-5 and np.isfinite(theta).all()),
                               max_gradient=error,newton_errors=history,initial=initial,penalized_training_loss=loss)
...
def main():
    spec=dict(question='Do common-input features improve masked-edge prediction beyond regularized node effects and distance?',
              predecessor_sha256=BASE.sha(HERE/'masked_neighbor_prediction_contract.json'), numerical_correction='Same objective; up to3Newton updates, unchanged1e-5gate', population='existing347Minnie23Pcells, threshold1; all60031dyads; both directions hidden together',
    ...
    cp=HERE/'masked_neighbor_prediction_polished_contract.json'
    ...
    source.source.run.shared.source.write_once(HERE/'masked_neighbor_prediction_polished_result.json',result)
