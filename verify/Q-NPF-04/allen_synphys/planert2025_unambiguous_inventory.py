...
save('planert2025_unambiguous_inventory_contract.json',{
    'revision':'Inventory only after excluding entire clusters with ANY duplicate directed pair; preserve ambiguous original records; no arbitrary within-pair choice',
...
for r in rows:pair_groups[r['cellid_pre'],r['cellid_post']].append(r)
ambiguous=sorted({r['clusterid'] for rr in pair_groups.values() if len(rr)>1 for r in rr})
original_rows=len(rows)
rows=[r for r in rows if r['clusterid'] not in ambiguous]
...
out={'contract_sha256':sha(HERE/'planert2025_unambiguous_inventory_contract.json'),
     'excluded_clusters':ambiguous,'excluded_rows':original_rows-len(rows),'cells':len(cells),'connection_rows':len(rows),'counts':dict(counts),
...
save('planert2025_unambiguous_inventory_result.json',out)
