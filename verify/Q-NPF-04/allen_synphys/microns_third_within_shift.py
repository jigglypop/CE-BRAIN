...
def main():
    specs = [('5_6','microns_third_trial_level_contract.json','microns_third_trial_level_result.json','microns_third_scan_contrast_pairs.json')]
    inputs = []
    ...
    save('microns_third_within_shift_contract.json',dict(
        ...
        selection='Third scan5/6; both timings; all10/first5/last5; all39 registered targets. All6 scenarios; no favorable selection.',
    ...
    save('microns_third_within_shift_result.json',dict(results=out,status='DESCRIPTIVE_WITHIN_TRIAL_ALIGNMENT_CONTROL',limits='Not calibrated p-values or independent animal replication.'))
    for r in out:
