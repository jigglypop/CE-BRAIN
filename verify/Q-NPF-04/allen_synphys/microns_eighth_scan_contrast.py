...
def main():
    save('microns_eighth_scan_contrast_contract.json',dict(question='Apply the existing first-window structural contrast to the next scan6/7 with58 nonoverlapping nuclei in the same mouse.',
    ...
    meta=json.loads((HERE/'microns_eighth_scan_result.json').read_text());assert meta['all_finite'] and meta['nonconstant']==58
    path=ROOT/'data/external/microns_functional_nwb/scan_6_7_first1250.npz';assert sha(path)==meta['artifact_sha256']
    ...
    for mode in ('common_frame','positive_delay_sensitivity'):
        ...
        assert len(pairs)==1653
        results.append(dict(timing=mode,cells=58,dyads=1653,**contrasts(pairs)))
        outputs.append(dict(timing=mode,pairs=pairs))
    save('microns_eighth_scan_contrast_pairs.json',outputs)
    save('microns_eighth_scan_contrast_result.json',dict(results=results,fixed_distance_boundaries_um=cuts.tolist(),limits='Conditional registered-cell descriptive observation in same mouse; not replicated causal mechanism.'))
