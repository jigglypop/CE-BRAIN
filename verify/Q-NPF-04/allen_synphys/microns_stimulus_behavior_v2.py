...
def main():
    ...
    save('microns_stimulus_behavior_v2_contract.json',dict(question='Which named stimulus trials and behavioral signals overlap the existing first1250 frames?',
    ...
    with raw.CachedRanges() as reader:
        with h5py.File(reader,'r') as f:
            ...
            for name,path in [('treadmill','acquisition/treadmill_velocity'),('pupil_major','acquisition/PupilTracking/pupil_major_radius'),('pupil_minor','acquisition/PupilTracking/pupil_minor_radius'),('eye','acquisition/EyeTracking/eye_position')]:
                ...
                assert t[-1]>=times[-1]
                covered=(times>=t[0])&(times<=t[-1])
                end=min(len(t),int(np.searchsorted(t,times[-1],side='right'))+1)
                t=t[:end];v=g['data'][:end]
                arrays[name+'_time']=t;arrays[name+'_value']=v
                metadata.append(dict(response_frames_covered=int(covered.sum()),response_frames_uncovered=int((~covered).sum()),name=name,path=path,shape=list(v.shape),time_start=float(t[0]),time_stop=float(t[-1]),finite=int(np.isfinite(v).sum()),total=int(v.size),unit=str(g['data'].attrs.get('unit','')),conversion=str(g['data'].attrs.get('conversion','')),offset=str(g['data'].attrs.get('offset',''))))
    ...
    save('microns_stimulus_behavior_v2_result.json',result)
