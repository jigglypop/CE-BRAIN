...
NAME = 'vc_amplitude_estimands_v2'
...
def functions():
    ...
    nodes = []
    names = {'_psp_inner', 'psp_func', '_compute_rise_tau', '_psp_max_time', 'stacked_psp_func'}
    ...
    return ns['Psp'].psp_func, ns['StackedPsp'].stacked_psp_func, ns['Psp']


def analyze(fits):
    psp, stacked, Psp = functions()
    ...
    for fit in fits:
        ...
        # 제작 파이프라인은 rise_power를 저장하지 않고 fit_psp는 2로 고정한다.
        assert params['rise_power'] is None or params['rise_power'] == 2
        params['rise_power'] = 2
        assert all(v is not None and np.isfinite(v) for v in params.values())
        valid_domain = params['rise_time'] < params['rise_power'] * params['decay_tau']
        ...
        rise_tau = Psp._compute_rise_tau(params['rise_time'], 2, params['decay_tau'])
        actual_peak_time = Psp._psp_max_time(rise_tau, 2, params['decay_tau']) + params['xoffset']
        actual_peak = psp(np.array([actual_peak_time]), **signal_args)[0]
        ...
        summaries.append(dict(group=group, fit=fit, n=60,
            ...
            model_valid_domain=bool(valid_domain), effective_rise_power=2,
            evaluated_peak_pa=float(actual_peak * 1e12), evaluated_peak_time_ms=float(actual_peak_time * 1000),
...
def main():
    ...
    inputs = [PSP, 'qc_sources/avg_response_fit.py', 'producer_average_membership_result.json',
        ...
        'source_snapshots/aisynphys__pipeline__multipatch__synapse.py']
    if args.verify:
    ...
    save(NAME + '_contract.json', dict(
        supersedes='v1은 fit_rise_power NULL로 비교 전 중단. 공개 저장 루프가 이 열을 기록하지 않고 fit_psp가 2로 고정함을 확인했다. v2는 이 조건을 명시하며 유효 영역 위반도 별도 보고한다. 재적합과 기준 완화는 없다.',
        ...
        decomposition='관측 대비=전체 적합 대비+잔차 대비; 전체 적합 대비=PSP 성분 대비+지수 기준선 대비. amp의 정점 해석은 rise_time<rise_power*decay_tau에서만 유효하다. 영역 밖은 소스의 clipping을 그대로 평가하고 실제 극값도 기록한다.',
