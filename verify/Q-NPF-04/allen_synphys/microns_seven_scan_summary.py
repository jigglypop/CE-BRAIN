...
def main():
    ...
    save('microns_seven_scan_summary_result.json',dict(results=rows,sources=sources,
        cells_by_scan={s:len(v) for s,v in ids.items()},unique_nuclei=len(set.union(*ids.values())),overlaps=overlaps,
        code_sha256=sha(Path(__file__)),limits='Same animal, scan-specific populations and denominators. No pooled significance test or independent animal replication. Includes all seven completed ordered scans; incomplete16-scan coverage.'))
