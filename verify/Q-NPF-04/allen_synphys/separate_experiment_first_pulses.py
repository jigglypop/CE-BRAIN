...
def main():
    selection=HERE/'separate_experiment_selection_result.json'
    selected=json.loads(selection.read_text(encoding='utf-8'))['selected'][0]
    save('separate_experiment_first_pulses_contract.json',dict(
        ...
        limits='Separate experiment, animal independence unverified; producer waveform parameters use unknown full-record scope. Not blinded discovery or raw reproduction.',
        selected_sha256=sha(selection),code_sha256=sha(Path(__file__))))
    ...
    save('separate_experiment_first_pulses_result.json',dict(contract_sha256=sha(HERE/'separate_experiment_first_pulses_contract.json'),selection=selected,records=rows,summary=summary,new_bytes=downloaded))
