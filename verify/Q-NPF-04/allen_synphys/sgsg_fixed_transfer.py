...
def main():
    ...
    spec = dict(
        ...
        seeds=list(range(202609051600, 202609051616)),
        ...
        archive_sha256=base.sha(ARCHIVE), code_sha256=base.sha(Path(__file__)),
    ...
    cp = HERE / 'sgsg_fixed_transfer_contract.json'
    write_once(cp, spec)
    ...
    result = dict(contract_sha256=base.sha(cp), observed=summary(observed, xyz, bins),
                  ...
                  records=records)
    write_once(HERE / 'sgsg_fixed_transfer_result.json', result)
