...
def main():
    ...
    save('microns_eighth_repeats_contract.json',dict(selection='scan6/7 all58 fixed targets, all Clip conditions appearing10 times; same .5<=relative_time<9.5 grid at median imaging interval, both timing conventions.',
    ...
    dest=ROOT/'data/external/microns_functional_nwb/scan_6_7_repeated_clips.npz'
    if dest.exists():
        receipt=json.loads((HERE/'microns_eighth_repeats_acquisition.json').read_text());assert sha(dest)==receipt['artifact_sha256'];print('NEXT_REPEATS_CACHE_VERIFIED');return
    ...
    save('microns_eighth_repeats_acquisition.json',dict(shape=list(aligned.shape),artifact_sha256=sha(dest),all_finite=bool(np.isfinite(aligned).all()),conditions=list(groups),same_condition_hashes_as_first=set(groups)==set(first),trial_start_min=min(t[2] for t in windows),trial_start_max=max(t[2] for t in windows)))
