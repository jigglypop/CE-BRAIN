...
def main():
    ...
    pairs=json.loads((HERE/'microns_eleventh_scan_contrast_pairs.json').read_text())[0]['pairs'];a=np.array([lookup[p['unit_a']] for p in pairs]);b=np.array([lookup[p['unit_b']] for p in pairs]);simple,matched=weights(pairs)
    ...
    output=HERE/'microns_eleventh_repeat_shift_draws.npz'
    if not output.exists():
        with output.open('xb') as f:np.savez_compressed(f,**draws)
    save('microns_eleventh_repeat_contrast_result.json',dict(results=results,controls=controls,code_sha256=sha(Path(__file__)),draws_sha256=sha(output),artifact_sha256=sha(path),limits='Same animal, partly overlapping registered nuclei. Historical registration uncertainty and common inputs remain. No behavior correction.'))
    for r in results:
