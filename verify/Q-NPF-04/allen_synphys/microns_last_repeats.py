...
def main():
    targets=json.loads((HERE/'microns_last_scan_result.json').read_text())['targets']
    save('microns_last_repeats_contract.json',dict(selection='scan9/6 all20 fixed targets, all Clip conditions appearing10 times; same .5<=relative_time<9.5 grid at median imaging interval, both timing conventions.',
    ...
    dest=ROOT/'data/external/microns_functional_nwb/scan_9_6_repeated_clips.npz'
    if dest.exists():
        receipt=json.loads((HERE/'microns_last_repeats_acquisition.json').read_text());assert sha(dest)==receipt['artifact_sha256'];print('NEXT_REPEATS_CACHE_VERIFIED');return
    raw.URL=next(u for u in json.loads((HERE/'microns_scan_9_6_asset.json').read_text())['contentUrl'] if u.startswith('https://dandiarchive.s3.amazonaws.com/'))
    raw.CACHE=ROOT/'data/external/microns_functional_nwb/scan_9_6_ranges';raw.LIMIT=160*1024*1024
    ...
    save('microns_last_repeats_acquisition.json',dict(shape=list(aligned.shape),artifact_sha256=sha(dest),all_finite=bool(np.isfinite(aligned).all()),conditions=list(groups),same_condition_hashes_as_first=set(groups)==set(first),trial_start_min=min(t[2] for t in windows),trial_start_max=max(t[2] for t in windows)))
