...
def main():
    ...
    with path.open('rb') as stream:
        ...
        while left:
            ...
            digest.update(block)
            left -= len(block)
    stem = f'rowland_{args.mouse}_run{args.run}'
    save(stem + '_contract.json', {
        ...
        'code_sha256': sha(HERE / 'rowland_session_comparison.py'),
    ...
    assert np.all(np.diff(data['galvo_ms'][idx]) > 0)
    ...
    result = {'contract_sha256': sha(HERE / (stem + '_contract.json')),
              ...
              'original_trials': len(data['galvo_ms']), 'original_indices': idx.tolist(),
    ...
    print(json.dumps({**{k: result[k] for k in ['mouse', 'run', 's1_cells', 's2_cells', 'trials', 'original_trials']},
