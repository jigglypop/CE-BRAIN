...
save('peng2024_half_mask_roles_contract.json',{
    'mask':'Within each record hash seed0|sorted cell pair; sort digests; alternating balanced folds 0,1; hide whole target fold; deterministic without labels',
    ...
    'feature':'for each target (a,b), omit its entire mask fold; r=(out_a-in_a)/(incident_a+1)-(out_b-in_b)/(incident_b+1) on other edges',
    'models':['pia difference/100 only','pia difference/100 and r'],
    'fit':'leave entire patient out of coefficient training; ridge identity, no intercept or tuning; each training target fold likewise omitted from its own features',
    'task':'conditional network completion with only opposite mask-fold orientations observed, not prediction of a fully unseen graph; no joint graph likelihood',
    'checks':'flipping every hidden orientation leaves features unchanged; vertex swap negates role; train/test patient split; gradient; reproduce earlier depth-only losses',
...
for cluster,indices in groups.items():
    ordered=sorted(indices,key=lambda i:hashlib.sha256(('seed0|'+edges[i]['a']+'|'+edges[i]['b']).encode()).hexdigest())
    for position,i in enumerate(ordered):mask[i]=position%2
    assert abs(sum(mask[i]==0 for i in indices)-sum(mask[i]==1 for i in indices))<=1
def role(target,a,b):
    net={a:0,b:0};degree={a:0,b:0}
    for j in groups[edges[target]['cluster']]:
        if mask[j]==mask[target]:continue
...
for i,e in enumerate(edges):
    ...
    hidden=[j for j in groups[e['cluster']] if mask[j]==mask[i]]
    for j in hidden:edges[j]['y']=1-edges[j]['y']
    assert role(i,e['a'],e['b'])==r
    for j in hidden:edges[j]['y']=1-edges[j]['y']
...
out={'contract_sha256':sha(HERE/'peng2024_half_mask_roles_contract.json'),'directions':len(edges),'patients':len(set(pids)),
     ...
     'folds':folds,'predictions':predictions,'features':features,'mask':[mask[i] for i in range(len(edges))],
     'no_visible_neighbor_targets':sum(not any(mask[j]!=mask[i] and ({edges[j]['a'],edges[j]['b']} & {e['a'],e['b']}) for j in groups[e['cluster']]) for i,e in enumerate(edges)),
     'verification':'PASS: whole-hidden-fold-flip exclusion, swap symmetry, patient separation, gradients and depth baseline reproduction'}
save('peng2024_half_mask_roles_result.json',out)
print(json.dumps({k:v for k,v in out.items() if k not in ('folds','predictions','features','mask')},indent=2))
