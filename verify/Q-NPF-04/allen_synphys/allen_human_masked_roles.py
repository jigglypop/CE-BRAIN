...
save('allen_human_masked_roles_contract.json',{
...
for seed in (0,1,2):
    ...
    results[str(seed)]={'metrics':metrics,'delta_logloss':float(delta.mean()),
        ...
        'nonzero_role_targets':sum(r!=0 for r in roles),'roles':roles,'mask':[mask[i] for i in range(len(edges))]}
    print(seed,json.dumps({name:{k:v for k,v in m.items() if k not in ('folds','probabilities')} for name,m in metrics.items()}),flush=True)
save('allen_human_masked_roles_result.json',{'contract_sha256':sha(HERE/'allen_human_masked_roles_contract.json'),
