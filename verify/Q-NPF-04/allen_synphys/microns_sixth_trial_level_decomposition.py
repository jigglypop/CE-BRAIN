...
def main():
    path=ROOT/'data/external/microns_functional_nwb/scan_6_4_repeated_clips.npz'
    receipt=json.loads((HERE/'microns_sixth_repeats_acquisition.json').read_text());assert sha(path)==receipt['artifact_sha256']
    save('microns_sixth_trial_level_contract.json',dict(question='How much of the pooled residual structural contrast comes from trial-level means versus within-trial fluctuations?',
        method='Same53 cells and fixed pair weights. First remove leave-one-repeat-out condition-time pattern, then decompose each trial into its57-point mean and centered within-trial component. Normalize component covariances by original total standard deviations to preserve exact additivity; also report separately renormalized within-trial correlations.',
    ...
    pairs=json.loads((HERE/'microns_sixth_scan_contrast_pairs.json').read_text())[0]['pairs'];_,w=weights(pairs)
    ...
    for mode,timing in enumerate(('common_frame','positive_delay_sensitivity')):
        for subset,repeats in [('all10',list(range(10))),('first5',list(range(5))),('last5',list(range(5,10)))]:
            ...
            total=r.reshape(-1,53);total=total-total.mean(axis=0)
            between=np.broadcast_to(means,r.shape).reshape(-1,53);between=between-between.mean(axis=0)
            within=within.reshape(-1,53);within=within-within.mean(axis=0)
    ...
    save('microns_sixth_trial_level_result.json',dict(results=out,status='EXACT_COVARIANCE_DECOMPOSITION',limits='Additive components use total variance denominators; separately normalized within correlation has a different estimand. No causal attribution.'))
    for r in out:
