"""Schur Newton execution of the preserved microns_node_metric.py design.

Original L-BFGS-B source and this solver are both input-hash checked.
"""
...
OUTPUT=HERE/"microns_node_metric_schur_result.json"
ARRAYS=HERE/"microns_node_metric_schur_arrays.npz"
CHECKPOINT=HERE/"microns_node_metric_schur_blocks.jsonl"
...
class Problem:
    ...
    def fit(self,features,signed=False,initial=None):
        ...
        return fit_newton(self,features,signed,initial)
...
def main():
    ...
    inputs[HERE/"microns_schur_solver.py"]="1fd75c944a10a80965cecd269c5df713a4e1f8ee7d899206977280d0d09e606b"
    ...
    result=dict(version="microns-node-metric-schur-v1",code_sha256=source_hash,input_sha256={p.relative_to(ROOT).as_posix():h for p,h in inputs.items()},arrays_sha256=sha(ARRAYS),checkpoint_sha256=sha(CHECKPOINT),pyarrow_version=pyarrow.__version__,
        settings=dict(selection="Prior 1348 column AND axon/dendrite proofread roots, unchanged",label="At least one annotation between distinct roots in selected export; zero means unlisted, not biological absence",
            voxel_size_nm=[4,4,40],position_unit="micrometers in raw EM image axes",length_unit_um=100.,context_ridge=1.,distance_penalty=0.,optimizer="Exact Hessian Schur Newton on the identical convex objective; no statistical settings changed",
            ...
            categories="Codebook labels from published metadata; fitted categories and offsets from training block only; unseen context offset=0; no degree or heldout edge features",
