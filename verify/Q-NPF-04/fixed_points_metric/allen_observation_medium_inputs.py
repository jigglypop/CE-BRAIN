...
def finite(value):return value is not None and bool(np.isfinite(value))
...
def main():
    ...
    state=dict(code_sha256=source_hash,input_sha256={p.relative_to(ROOT).as_posix():v for p,v in expected.items()},
               previous_failure={p.name:sha(p) for p in [HERE/"allen_observation_medium_serialization_failure.py",HERE/"allen_observation_medium_serialization_failure_state.json"] if p.exists()},
