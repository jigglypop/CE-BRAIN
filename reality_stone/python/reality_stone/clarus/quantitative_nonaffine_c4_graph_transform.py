"""Exact C4 certificate for a graph-independent nonaffine triangular base."""
...
if __package__:
    from .quantitative_nonaffine_c3_graph_transform import (
    ...
    from quantitative_nonaffine_c3_graph_transform import (  # type: ignore[no-redef]
        QuantitativeNonaffineC3GraphTransformCertificate,
        quantitative_nonaffine_c3_triangular_graph_transform,
    )
...
class NonaffineBaseC4Bounds:
    inverse_derivative_upper: Fraction
    inverse_hessian_upper: Fraction
    inverse_third_derivative_upper: Fraction
    inverse_fourth_derivative_upper: Fraction


@dataclass(frozen=True)
class QuantitativeNonaffineC4GraphTransformCertificate:
    status: str
    validation_level: str | None
    failure_codes: tuple[str, ...]
    robust_interior: bool
    nonaffine_c3_certificate: QuantitativeNonaffineC3GraphTransformCertificate
    base_inverse_fourth_derivative_upper: Fraction
...
@dataclass(frozen=True)
class NonaffineC4GraphIterationBound:
    steps: int
...
def sine_perturbed_base_c4_bounds(amplitude_upper: object) -> NonaffineBaseC4Bounds:
    """Scalar inverse C4 bounds for phi(x)=x+a sin(x), on 0 <= a < 1."""
    ...
    return NonaffineBaseC4Bounds(
        ...
        inverse_fourth_derivative_upper=(
            amplitude / gap**5
            + 10 * amplitude**2 / gap**6
            + 15 * amplitude**3 / gap**7
        ),
    )


def quantitative_nonaffine_c4_triangular_graph_transform(
    *,
    base_dimension: int,
    ...
    base_inverse_fourth_derivative_upper: object,
    ...
    """Compose the exact nonaffine C3 gate with the fourth inverse chain rule."""
    upsilon = _exact_fraction(
    ...
    if min(upsilon, k4, k5, lambda4) < 0:
        raise ValueError("normalized inverse/map/graph C4 bounds must be nonnegative")

    c3 = quantitative_nonaffine_c3_triangular_graph_transform(
        ...
        base_inverse_third_derivative_upper=base_inverse_third_derivative_upper,
    ...
    tau = c3.base_inverse_third_derivative_upper
    ...
    r = 1 + kappa
    s = q * kappa + lipschitz.normalized_base_to_fiber_lipschitz
    ...
    inverse_second_weight = 3 * nu**2 + 4 * mu * tau
    output_fourth = (
        mu**4 * a4
        + 6 * mu**2 * nu * a3
        + inverse_second_weight * a2
        + upsilon * s
    )
    ...
    a4_f = 4 * k2 * r
    a4_e = 6 * k2 * lambda2 + 6 * k3 * r**2
    a4_d = 4 * k2 * lambda3 + 12 * k3 * lambda2 * r + 4 * k4 * r**3
    a4_delta = (
        k2 * lambda4
        + 4 * k3 * lambda3 * r
        + 3 * k3 * lambda2**2
    ...
    a3_e = 3 * k2 * r
    a3_d = 3 * (k2 * lambda2 + k3 * r**2)
    a3_delta = k2 * lambda3 + 3 * k3 * lambda2 * r + k4 * r**3
    a2_d = 2 * k2 * r
    a2_delta = k2 * lambda2 + k3 * r**2
    a1_delta = hy * kappa + hx

    c43 = mu**4 * a4_f + 6 * mu**2 * nu * q
    c42 = mu**4 * a4_e + 6 * mu**2 * nu * a3_e + inverse_second_weight * q
    c41 = (
        mu**4 * a4_d
        + 6 * mu**2 * nu * a3_d
        + inverse_second_weight * a2_d
        + upsilon * q
    )
    c40 = (
        mu**4 * a4_delta
        + 6 * mu**2 * nu * a3_delta
        + inverse_second_weight * a2_delta
        + upsilon * a1_delta
    )

    failures = list(c3.failure_codes)
    if class_margin < 0:
        failures.append("NONAFFINE_C4_GRAPH_FOURTH_DERIVATIVE_CLASS_NOT_INVARIANT")
    if bunching_margin <= 0:
        failures.append("NONAFFINE_C4_FOURTH_DERIVATIVE_BUNCHING_NOT_STRICT")
    common = dict(
        failure_codes=tuple(failures),
        nonaffine_c3_certificate=c3,
    ...
    if failures:
        return QuantitativeNonaffineC4GraphTransformCertificate(
            status=failures[0], validation_level=None, robust_interior=False,
            c4_graph_real_dimension=None, **common,
        )
    return QuantitativeNonaffineC4GraphTransformCertificate(
        status="VERIFIED_QUANTITATIVE_NONAFFINE_C4_TRIANGULAR_GRAPH_TRANSFORM",
        validation_level="VERIFIED_QUANTITATIVE_NONAFFINE_C4_TRIANGULAR_GRAPH_TRANSFORM",
...
def nonaffine_c4_graph_iteration_bound(
    certificate: QuantitativeNonaffineC4GraphTransformCertificate,
    *,
    ...
    steps: int,
) -> NonaffineC4GraphIterationBound:
    """Iterate the exact common-inverse C0/C1/C2/C3/C4 recurrence."""
    if certificate.validation_level is None:
        raise ValueError("C4 iteration requires a verified nonaffine C4 certificate")
    ...
    c3 = certificate.nonaffine_c3_certificate
    c2 = c3.nonaffine_c2_certificate
    c1 = c2.c1_certificate
    ...
    for _ in range(steps):
        ...
        delta, derivative, hessian, third, fourth = (
            q * delta, next_derivative, next_hessian, next_third, next_fourth
        )
    return NonaffineC4GraphIterationBound(steps, delta, derivative, hessian, third, fourth)


__all__ = [
    "NonaffineBaseC4Bounds",
    "NonaffineC4GraphIterationBound",
    "QuantitativeNonaffineC4GraphTransformCertificate",
    "nonaffine_c4_graph_iteration_bound",
    "quantitative_nonaffine_c4_triangular_graph_transform",
    "sine_perturbed_base_c4_bounds",
]
