"""Exact C3 certificate for an affine coupled-base graph transform."""
...
if __package__:
    from .quantitative_coupled_c2_graph_transform import (
    ...
    from quantitative_coupled_c2_graph_transform import (  # type: ignore[no-redef]
        QuantitativeCoupledC2GraphTransformCertificate,
        quantitative_coupled_c2_graph_transform,
    )
...
class QuantitativeCoupledC3GraphTransformCertificate:
    status: str
    validation_level: str | None
    failure_codes: tuple[str, ...]
    robust_interior: bool
    coupled_c2_certificate: QuantitativeCoupledC2GraphTransformCertificate
...
@dataclass(frozen=True)
class CoupledC3GraphIterationBound:
    steps: int
...
def quantitative_coupled_c3_graph_transform(
    *,
    base_dimension: int,
    ...
    """Compose coupled C2 with exact C3/C3,1 invariance and bunching.

    The D3-map bounds supply the C2 predecessor's Hessian moduli.  The new
    D3-map fiber/input Lipschitz bounds are D4-level hypotheses, and the graph
    D3 modulus is required exactly when the base depends on graph height.
    """
    uf = _exact_fraction(
    ...
    if min(uf, ug, lambda3, xi3) < 0:
        raise ValueError("normalized C3 and D4-level modulus bounds must be nonnegative")

    c2 = quantitative_coupled_c2_graph_transform(
    ...
    base = c2.coupled_c1_certificate.coupled_lipschitz_certificate
    xi3_required = base.normalized_fiber_to_base_lipschitz > 0
    common = dict(
        coupled_c2_certificate=c2,
    ...
    if alpha <= 0 or q_value is None or c2.validation_level is None:
        ...
        return QuantitativeCoupledC3GraphTransformCertificate(
    ...
    adelta = hf * z * r + lfy * lambda2 * rx

    cu = lfy * lambda3 + 3 * hf * lambda2 * r + tf * r**3
    ...
    cu_lip = (
        lfy * xi3 + 4 * hf * r * lambda3 + 3 * hf * lambda2**2
    ...
    if lambda3_margin < 0:
        failures.append("COUPLED_C3_GRAPH_THIRD_DERIVATIVE_CLASS_NOT_INVARIANT")
    if xi3_required and xi3_margin < 0:
        failures.append("COUPLED_C31_GRAPH_CLASS_NOT_INVARIANT")
    if beta3_margin <= 0:
        failures.append("COUPLED_C3_THIRD_DERIVATIVE_BUNCHING_NOT_STRICT")
    detailed = dict(
    ...
    return QuantitativeCoupledC3GraphTransformCertificate(
        status="VERIFIED_QUANTITATIVE_COUPLED_C3_GRAPH_TRANSFORM",
        validation_level="VERIFIED_QUANTITATIVE_COUPLED_C3_GRAPH_TRANSFORM",
        robust_interior=robust, c3_graph_real_dimension=base_dimension,
        **detailed, **common,
    )


def coupled_c3_graph_iteration_bound(
    certificate: QuantitativeCoupledC3GraphTransformCertificate,
    *, initial_value_distance: object, initial_derivative_distance: object,
    ...
    steps: int,
) -> CoupledC3GraphIterationBound:
    """Iterate the exact coupled C0/C1/C2/C3 upper-triangular recurrence."""
    if certificate.validation_level is None:
        raise ValueError("iteration requires a verified coupled C3 certificate")
    ...
    c2 = certificate.coupled_c2_certificate
    c1 = c2.coupled_c1_certificate
    ...
    return CoupledC3GraphIterationBound(steps, delta, derivative, hessian, third)


__all__ = [
    "CoupledC3GraphIterationBound",
    "QuantitativeCoupledC3GraphTransformCertificate",
    "coupled_c3_graph_iteration_bound",
