"""BA-STAGE6 latency discrimination screen (scalar x_lat target).

Contract: paper/검증_원장/BA_STAGE6_지연_endpoint_계약.md. Scalar adaptation of
...
HERE = Path.cwd()
OUT = HERE / "ba-stage6-result.json"
LAT = json.loads((HERE / "ba-stage6-latency-endpoints.json").read_text(encoding="utf-8"))
...
MARGIN, TIE = 0.005, 0.002
PERM_N, PERM_SEED = 499, 20260832
...
def load_sources():
    ...
    for stage in ("d0", "d1", "d2", "d3"):
        ...
        for s in d["sources"]:
            ...
            if not ok or len(recs["anchor"]) != 4:
...
def pairwise(losses, left, right, seed_off):
    ...
    rng = np.random.Generator(np.random.PCG64(20260832 + seed_off))
...
def main():
    ...
    def beats(a, b):
        r = pw[f"{a}>{b}"]
        return r["mean_improvement"] >= MARGIN and r["lower_95"] > 0.0

    if not beats("D", "T"):
        decision = "STAGE6_LATENCY_NOT_MEASURABLE"
    elif beats("DP", "D") and perm_frac >= 0.95 and pw["RESID>D"]["mean_improvement"] > 0.0:
        decision = "STAGE6_LATENCY_ANATOMY_SUPPORTED"
    elif beats("DP", "D"):
        decision = "STAGE6_CONTROL_FAILED"
    else:
        decision = "STAGE6_DISTANCE_ONLY"

    result = {
        "schema": "ba-stage6-screen-v1",
        "contract": "paper/검증_원장/BA_STAGE6_지연_endpoint_계약.md",
        "latency_endpoints_sha256": hashlib.sha256(
            (HERE / "ba-stage6-latency-endpoints.json").read_bytes()).hexdigest(),
