"""BA-STAGE6 latency-endpoint extractor (byte-locked re-fetch, no raw persisted).
...
HERE = Path.cwd()
OUT = HERE / "ba-stage6-latency-endpoints.json"
RECEIPT = HERE / "ba-stage6-extract-receipt.json"
...
def x_lat_from_mean(w, masks):
    ...
    if not np.isfinite(total) or total <= 0:
        return None
    c = np.cumsum(e)
    t = float(tau[int(np.searchsorted(c, 0.5 * total))])
    return t / 0.050
...
def process_source(key, rows, meta, record):
    ...
    for t, info in zip(targets, tinfo):
        ...
        x = x_lat_from_mean(w, masks)
        out.append({"site_id": t["site_id"], "role": t["role"],
                    "x_lat": x, "max_z": t["max_z"],
                    "reason": None if x is not None else "ZERO_WINDOW_ENERGY"})
    return out
