...
M = _load("ce_verified_polynomial_radial_rank_bridge", "verified_polynomial_radial_polygon_rank_bridge.py")
...
def _bridge(**overrides):
    ...
    return M.verified_polynomial_radial_rank_via_inscribed_polygon(**values)


def test_quadratic_smooth_rank_one_is_verified_via_polygon_homotopy() -> None:
    result = _bridge()
    assert result.validation_level == "VERIFIED_POLYNOMIAL_RADIAL_POLYGON_HOMOTOPY_RIESZ_RANK"
    assert result.certified_nominal_rank == 1
    assert result.polynomial_family_rank_verified
    assert result.sector_homotopy_resolvent_verified
    assert result.smooth_contour_quadrature_rank_verified
    assert result.polynomial_construction.contour.genuinely_higher_order_radial_contour


def test_polygon_vertices_and_homotopy_cover_are_exact_receipts() -> None:
    result = _bridge()
    scale = result.polynomial_construction.spectral_reference_scale
    contour = result.polynomial_construction.contour
...
def test_homotopy_margin_is_polynomial_residual_margin() -> None:
    result = _bridge()
    assert result.polynomial_certificate is not None
    assert result.normalized_sector_homotopy_margin_lower == (
        result.polynomial_certificate.normalized_robust_delta_lower
    )
    assert result.normalized_sector_homotopy_margin_lower > 0


def test_projector_approximation_encloses_smooth_polynomial_projector() -> None:
    result = _bridge()
    assert result.polynomial_projector_quadrature is not None
    assert result.polynomial_projector_operator_error_upper is not None
    entry = result.polynomial_projector_quadrature[0][0]
    error = result.polynomial_projector_operator_error_upper
    assert abs(entry.real - 1) <= error
    assert abs(entry.imag) <= error


def test_outside_scalar_has_polynomial_radial_rank_zero() -> None:
...
def test_interval_family_inherits_polynomial_radial_rank() -> None:
    result = _bridge(uncertainty_radii=(((F(1, 20), 0),),))
    assert result.certified_nominal_rank == 1
    assert result.polynomial_certificate is not None
    assert result.polynomial_certificate.rank_preserved_for_entire_family
    assert result.polynomial_family_rank_verified
...
def test_zero_budget_keeps_high_multiplicity_rank_unresolved() -> None:
    ...
    assert result.status == "POLYNOMIAL_RADIAL_POLYGON_HOMOTOPY_RANK_UNRESOLVED"
    assert result.validation_level is None
    assert result.certified_nominal_rank is None
    assert result.sector_homotopy_resolvent_verified
    assert result.polynomial_projector_quadrature is None
...
def test_failed_cover_stops_before_polygon_quadrature() -> None:
    ...
    assert result.validation_level is None
    assert result.polynomial_certificate is not None
    assert result.polynomial_certificate.status == "VERIFIED_POLYNOMIAL_RADIAL_RESIDUAL_COVER_NONPOSITIVE"
    assert result.adaptive_polygon is None
    assert not result.sector_homotopy_resolvent_verified


def test_singular_polynomial_node_stops_before_residual_and_quadrature() -> None:
    result = _bridge(nominal_transition=((F(17, 16),),))
    assert result.status == "EXACT_POLYNOMIAL_RADIAL_NOMINAL_NODE_SINGULAR"
    assert result.polynomial_certificate is None
    assert result.adaptive_polygon is None
...
def test_invalid_polynomial_geometry_and_honesty_flags_remain_separate() -> None:
    with pytest.raises(ValueError, match="strictly exceed"):
        _bridge(radial_base=F(1, 8), radial_terms=((2, 0, F(1, 4)),))
    result = _bridge()
