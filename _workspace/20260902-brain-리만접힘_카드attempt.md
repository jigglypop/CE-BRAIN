...
## 하네스 4 (2026-09-04, Fable 5.1 지침 반영 + 커넥톰)
- 사용자가 Fable 5.1 프롬프팅 지침을 제공. 이 저장소에 걸리는 항목만 CLAUDE.md `## 운영`에 반영: **서브에이전트 블로킹 대기 금지**(이 세션에서 실제로 대기 구간이 길었다), 독립 호출 묶기, 파일 부분 수정 우선, 요청 범위 밖 작업 금지, 다음 단계 설명만 하고 턴 종료 금지, 압축 요약에 제약·결정·숫자 보존.
- 안전장치 오탐 항목은 이미 처리(원인은 메인 문맥 누적, 전담 에이전트 출력 계약을 파일 보고로 변경).
- **커넥톰 후보(사용자 제시).** 초파리 수컷 전뇌 연결지도가 세 카드 연속 기각의 핵심 원인을 직접 겨냥한다. 효능·지연이 생성모형의 숨은 값이 아니라 **측정 공변량**(edge별 연결 강도, 경로길이)이 되면 채널 식별 인증서 P3가 처음으로 통과 가능. 부가 효과로 직전 감사의 "예측 숫자가 저자 손잡이의 되읽기" P1이 해소된다. 한계: 단일 개체·정적이라 학습 전후 ΔW·Δτ 없음, 종이 달라 포유류 자료와 합산 불가 → 쓰임은 구조 입력·보정이지 학습 전후 판정 자료가 아니다. bio-reader가 메타데이터만으로 필드·접근·정합 확인 중, 보고 `verify/Q-NPF-04/bio-reader_connectome_eligibility.json`.

## 범위 확장과 후보 재편 (2026-09-04)
- **네 번째 인증서도 시간 채널 0.** 접속·측지선 후보의 채널 응답 z = 0.07 / 0.10 / 0.18(기준 2). 강도·이득은 통과(z 9.6·86.7·139.3, 7.5·7.7·4.0)하고 두 방향 각도 46도. 강도–시간 각도는 23도로 좁다. 산출물 `verify/Q-NPF-04/F-01/result_c13_geodesic_cert.json`.
- 진단: 통계량을 만들기 전 저차원 좌표 투영이 쌍별 상대 시차 재배치를 기저 회전으로 흡수해 상쇄시킨다. 그리고 시간 변화는 이력 효과인데 순간 좌표에는 이력이 없다.
- **구조 자료 판정(전담 보고).** 계량 endpoint 적격성 0/4. 세 카드의 P3 실패는 통계량의 성질이지 파라미터 값의 성질이 아니므로 측정값을 꽂는 것만으로 복구되지 않는다. 저자 자유도는 7개에서 4개로 축소(실질적이지만 인증서 아님). 절대 전달 속도는 1차 측정이 단일 세포형뿐이라 미확정, 고정되는 것은 상대 구조뿐. 탐색 표본 85MB 이하, 전체 표 1.1GB는 승인 필요. 보고 `verify/Q-NPF-04/bio-reader_connectome_eligibility.json`.
- 부산물: 같은 종의 완전 구조 지도가 복수 존재 → 개체 간 분산 바닥을 처음 측정 가능. 학습 자료 불필요. **Q-NPF-05로 등록**(priority 2).
- **사용자 범위 확장(2026-09-04, 표적 수준).** 계를 국소 영역이 아니라 머리에서 아랫배 감각까지의 중추신경계 전체로 본다. 이것이 시간 채널이 계속 0이었던 이유를 설명한다 — 국소에서는 미세 섭동, 긴 축 고리에서는 일차 항. 진전 원장 §2에 기록.
- **후보 재편(spec.md §후보 순위).** C1 긴 축 고리 공명 `1/f = 2(L/v + τ_syn) + O(τ_m)`, 기울기 2, 자유 진폭 없음, 채널이 주파수·감쇠비·기저 전력으로 구성상 분리. C2 쌍별 시차 분해(좌표 없음, 반대칭/결합크기/전력). C3 이력 포함 좌표 위의 계량(블록 최대 위치 이동 Δτ/h). C1 인증서 진행 중, 보고 `verify/Q-NPF-04/F-01/prover_loop_report.json`.
