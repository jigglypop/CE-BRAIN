...
is an eigenfunction with (\lambda_{A,n}=((n+\tfrac12)\pi)^2). All centre values are zero, hence continuous. The centre flux sum is ((n+\tfrac12)\pi\sum_ec_e=0), and the distal derivative vanishes because μos((n+\tfrac12)\pi=0). For unequal conductance weights the correct constraint would be ∑e (a_ec_e=0); E2 deliberately freezes equal edges and equal weights.
